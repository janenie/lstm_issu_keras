...
def canonicalize_word(word, unknown="UUUNKKK", wordset=None, digits=True):
    word = word.lower()
    if digits:
        if (wordset != None) and (word in wordset): return word
        word = canonicalize_digits(word) # try to canonicalize numbers
    if (wordset == None) or (word in wordset): return word
    else: return unknown # unknown token
...
def load_modified_dataset(fname):
    docs = []
...
def load_weight(fname):
    ...
    with open(fname) as fd:
        for line in fd:
            weights.append([float(e) for e in line.split(' ')])
        fd.close()

    return np.array(weights)

def write_weight(fname, arr):
    res = ''

    with open(fname, 'w') as fout:
        for a in xrange(arr.shape[0]):
            res += ' '.join(str(e) for e in arr[a]) + '\n'
...
def write_res(fname, res):
    ...
    with open(fname, 'w') as fout:
        for i in xrange(len(res)/2):
            line += "Unadjusted: " + str(res[2*i]) + "\n"
            line += "Adjusted for missing vocab: " + str(res[2*i+1]) + "\n"
        
        fout.write(line)
        fout.close()


def get_tfidf(vocab_size, vocab, docs):
    idf_size = {}
    for doc in docs:
        word_max_index = max(doc)
        record = zeros((word_max_index+1, ))
        for word in doc:
            if record[word] == 0:
                if idf_size.has_key(word):
                    idf_size[word] += 1
                    record[word] = 1
                else:
                    idf_size[word] = 1
    ...
    res = zeros((vocab_size+1, ))
    doc_num = len(docs)
    print "doc_num %d, frequent word idf %f" % (doc_num, idf_size[1])
    for k, v in idf_size.items():
        idf = math.log((doc_num+0.0) / (v+0.0) + 1.0) / math.log(2) 
        #tf = vocab['freq'][k]
        #res[k] = tf*idf
        res[k] = idf
    return res 
...
def docs_to_windows(docs, word_to_num, tag_to_num, wsize=3, unknown="UUUNKKK"):
    pad = (wsize - 1)/2
    docs = flatten1([pad_sequence(seq, left=pad, right=pad) for seq in docs])

    words, tags = zip(*docs)
    words = [canonicalize_word(w, unknown, word_to_num) for w in words]
...
def docs_to_lm_windows(docs, word_to_num, ngram=2, unknown="UUUNKKK"):
    docs = flatten1([pad_sequence(seq, left=(ngram-1), right=1)
                     for seq in docs])
    words = [canonicalize_word(wt[0], unknown, word_to_num) for wt in docs]
...
def add_pad_sequence(seq, left=1, right=1):
    return left*["<s>"] + seq + right*["</s>"]

def docs_to_index(docs, word_to_num, unknown="UUUNKKK"):
    docs = [add_pad_sequence(seq, left=1, right=1) for seq in docs]
    print docs[0]
    ret = []
    for seq in docs:
        words = [canonicalize_word(wt, unknown, word_to_num) for wt in seq]
        ret.append(seq_to_indices(words, word_to_num))

    return array(ret, dtype=object)

def docs_to_indices(docs, word_to_num, unknown="UUUNKKK"):
    docs = [pad_sequence(seq, left=1, right=1) for seq in docs]
    ret = []
    #print docs[0]
    for seq in docs:
        words = [canonicalize_word(wt[0], unknown, word_to_num) for wt in seq]
        ret.append(seq_to_indices(words, word_to_num))
...
def docs_to_tag_sequence(docs, word_to_num, tag_to_num, unknown="UUUNKKK"):
    # docs = [pad_sequence(seq, left=1, right=1) for seq in docs]
    X = []
    Y = []
    for seq in docs:
